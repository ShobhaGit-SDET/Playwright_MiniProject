...
(0, test_1.test)('[Basic Auth - Success] Postman Echo basic auth with correct credentials', async ({ request }) => {
    ...
    const res = await request.get(`${process.env.BASE_URL}/basic-auth`, {
        headers: {
            Authorization: (0, auth_utils_1.basicAuthHeader)(process.env.USER, process.env.PASS),
...
(0, test_1.test)('[Basic Auth - Failure] Postman Echo basic auth with WRONG credentials should be 401', async ({ request }) => {
    ...
    const res = await request.get(`${process.env.BASE_URL}/basic-auth`, {
        headers: {
            Authorization: (0, auth_utils_1.basicAuthHeader)(process.env.USER, process.env.WRONG_PASS),
            Accept: 'application/json',
        },
    });
...
//# sourceMappingURL=basicAuthwithenv.spec.js.map